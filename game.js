...
// Returns 1 inside the track ring, 0 on grass, with a smooth falloff across
// ~1 cell width at each edge so the boundary isn't a hard per-cell step.
function trackAmount(cellX, cellZ) {
  ...
  var edgeSoftness = 1 / minRadius;
  var dist = Math.abs(d - 1) - ringHalfWidth; // <0 inside track, >0 outside
  return 1 - Math.max(0, Math.min(1, dist / edgeSoftness));
}
...
function main() {
  ...
  var unicornRoot = makeNode(unicornNodeDescriptions);
  unicornRoot.setParent(playerRoot);
  ...
  function drawScene(timeMs) {
    ...
    // ---- Gallop animation (only while moving) ----
    var speed = 3;
    var c = time * speed;
    var gait = moving ? 1 : 0;
    var adjust;
