...
function main() {
  ...
  var bodyBufferInfo = createCubeBufferInfo(gl, 1, [0.95, 0.92, 0.98, 1]); // white/lavender coat
  var hornBufferInfo = createCubeBufferInfo(gl, 1, [1.0, 0.85, 0.3, 1]);   // gold horn
  var maneBufferInfo = createCubeBufferInfo(gl, 1, [0.85, 0.55, 0.95, 1]); // pink/purple mane+tail
  ...
  // Quadruped rig, described in the animal's own local space:
  // +Z points toward the animal's head (forward), +Y is up.
  // Legs hang from the body at the four corners.
  var unicornNodeDescriptions = {
    name: "root",
    draw: false,
    children: [
      { name: "body", translation: [0, 2.5, 0], children: [
        { name: "neck", translation: [0, 0.9, 1.1], scale: [1, 1, 1], children: [
  ...
  var scene = makeNode(unicornNodeDescriptions);
  ...
  function drawScene(time) {
    ...
    // Camera behind the unicorn, looking toward its head (+Z), for a
    // "view from behind" over its back.
    var cameraPosition = [7, 3.5, -9];
    var target = [0, 1, 1];
    var up = [0, 1, 0];
    ...
    // Gallop: body bob + front/back leg pairs swinging in opposition.
    adjust = Math.abs(Math.sin(c * 2)) * 0.3;
    ...
    adjust = Math.sin(c * 2) * 0.15;
    nodeInfosByName["neck"].trs.rotation[0] = adjust;
    adjust = Math.sin(c * 2 + 0.2) * 0.1;
